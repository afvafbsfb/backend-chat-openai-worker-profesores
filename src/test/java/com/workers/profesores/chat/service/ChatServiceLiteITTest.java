 ...
 */
public class ChatServiceLiteITTest {
    @Test
    public void testSecondTurnLiteUsuarios() throws Exception {
        ...
        java.lang.reflect.Field fDebug = ChatService.class.getDeclaredField("debug");
        fDebug.setAccessible(true);
        fDebug.set(chat, true);
        ...
        // Stub whitelist lookup for endpoint used by the tool
     ...
     // Stub both overloads (with and without xmlLogger)
     Mockito.when(apiProxy.executeSpecCall(Mockito.anyMap(), Mockito.eq("GET"), Mockito.any(), Mockito.any(), Mockito.any(), Mockito.anyString(), Mockito.any()))
         .thenReturn(apiResult);
     Mockito.when(apiProxy.executeSpecCall(Mockito.anyMap(), Mockito.eq("GET"), Mockito.any(), Mockito.any(), Mockito.any(), Mockito.anyString()))
         .thenReturn(apiResult);
        ...
        // Debug aid for CI logs
        System.out.println("[TEST][LiteIT] data.type=" + out.getData().getType());
    }
}
